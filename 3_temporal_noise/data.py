...
for idt, t in enumerate(T):
    ...
    for idn, n in enumerate(N):
        ...
        out[idt, idn, 3:] = opt_uncertainty_squeezedramsey_tcor_separable(n, t, t_cor, t0= t0_sep, t_lim=[t_max / (1.1 * np.sqrt(n)), 1.1 * t_max], th_t= th_t, method= method)
        _, k0, t0, _, _, t0_sep = out[idt, idn]
